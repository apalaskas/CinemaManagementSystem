...
Rate limits are configured under `cinema.rate-limit` for Program search, Screening search, creation, and Screening submission. The in-process map has an explicit maximum key count and idle-entry TTL. The rate-limit filter runs before HTTP Basic authentication, as required by SD-2, so an identity already established by an upstream trusted security mechanism is keyed by domain user ID and otherwise the request is keyed by the servlet container's directly resolved remote address. Untrusted forwarding headers are not accepted implicitly. This implementation is suitable only for the single-instance academic deployment. A distributed deployment must replace it with a shared limiter; Redis, containers, and other distributed infrastructure are intentionally absent.
