...
| FR-2 | Program details, PROGRAMMER roles, deletion; UC-P2 | `PATCH /programs/{id}`; `DELETE /programs/{id}`; `POST /programs/{id}/roles`; `DELETE /programs/{id}/roles/{userId}` | Program Service; Context-Aware Authorization Service; Program/User-Role repositories; Audit Logging Service | Detail/role/delete service state tables; creator-removal denial; concurrency tests; controller authorization/error tests; MySQL role uniqueness IT | PLANNED |
| FR-3 | STAFF management/freeze; UC-P2 | `POST /programs/{id}/roles`; `DELETE /programs/{id}/roles/{userId}`; Program transition endpoint | Program Service; Context-Aware Authorization Service; User and Role Repository; Audit Logging Service | CREATED-only STAFF mutation tests; role-conflict tests; freeze-on-SUBMISSION transaction IT | PLANNED |
| FR-4 | Strict Program lifecycle; UC-P3 | `POST /api/v1/programs/{id}/transitions` | Program REST Controller; Program Service; Screening Service collaborator; Program/Screening repositories; Transaction Manager; Audit Logging Service | Full legal transition parameterized tests; skip/rollback/prerequisite/concurrency tests; automatic rejection/rollback MySQL IT | PLANNED |
| FR-5 | Screening draft creation/update; UC-S1 | `POST /programs/{id}/screenings`; `PATCH /screenings/{id}` | Screening REST Controller; Screening Service; Validation Service; Authorization Service; Idempotency/Audit services; Screening/User-Role repositories | Partial-valid-draft and owner update tests; programmer/staff exclusion; explicit-time validation; candidate-overbooking tests; controller DTO tests | PLANNED |
| FR-6 | Submission and withdrawal; UC-S1, UC-S2 | `POST /screenings/{id}/submit`; `DELETE /screenings/{id}` | Screening Service; Rate Limiter; Idempotency Manager; Authorization Service; Screening/Program repositories; Audit Logging Service | Completeness/state/ownership tests; CREATED and pre-ASSIGNMENT SUBMITTED withdrawal matrix; soft-delete query tests; idempotent replay and rollback IT | PLANNED |
| FR-7 | Handler assignment/review; UC-S3 | `POST /screenings/{id}/handler`; `POST /screenings/{id}/review` | Screening Service; Authorization Service; User and Role Repository; Screening Repository; Audit Logging Service | Frozen-STAFF/exactly-one handler tests; assigned-handler-only review; score boundary/comment tests; one-review unique constraint IT | PLANNED |
| FR-8 | Decision, final submission, scheduling, automatic rejection; UC-S4 and UC-P3 | `POST /screenings/{id}/decision`; `/final-submission`; `/schedule`; Program transition endpoint | Screening Service; Schedule Conflict Checker; Program/Screening repositories; Transaction Manager; Audit Logging Service | PROGRAMMER decision/reason tests; owner final-submit/freeze tests; half-open adjacent/overlap tests; case-insensitive auditorium and concurrent scheduling MySQL IT; automatic rejection tests | PLANNED |
| FR-9 | Program search; UC-P4 | `GET /api/v1/programs`; `GET /api/v1/programs/{id}` | Search and View REST Controller; Search and Visibility Service; Program Repository | Filter AND/case/trim/sort/page tests; anonymous vs managed visibility MockMvc tests; MySQL query/projection and stable pagination IT | PLANNED |
| FR-10 | Screening search; UC-S5 | `GET /api/v1/programs/{id}/screenings`; `GET /api/v1/screenings/{id}` | Search and View REST Controller; Search and Visibility Service; Screening Repository | All-word per-field search tests; filter AND semantics; general/timetable sort; page stability; MySQL execution IT | PLANNED |
| FR-11 | Redaction/visibility; UC-P4, UC-S5 | All four GET search/view endpoints | Search and Visibility Service; Context-Aware Authorization Service; query projections; Global Exception Handler | Role-by-field projection matrix; pre/post-decision Review redaction; concealed 404; soft-delete/public projection MockMvc and MySQL IT | PLANNED |
...
| UC-P2 | Manage Program Details and Roles | FR-2, FR-3; NFR-2/3/4 | Program PATCH/DELETE and role POST/DELETE | Program Service and User/Role Repository; AD-2 branches, creator protection, STAFF freeze, transaction/audit tests | PLANNED |
| UC-P3 | Advance Program Lifecycle | FR-4, FR-8.5; NFR-2/3/4 | `POST /api/v1/programs/{id}/transitions` | Locked Program Service transition orchestration; SD-1 legal/prerequisite/auto-reject/rollback/concurrency tests | PLANNED |
| UC-P4 | Search and View Programs | FR-9, FR-11; NFR-1/3 | Program collection/detail GET | Search/Visibility Service and SQL projections; AD-4 filter/access/redaction/sort/page tests | PLANNED |
| UC-S1 | Prepare Screening | FR-5, FR-6.3; NFR-2/3/4 | Screening create/PATCH/DELETE | Screening Service; AD-3 partial draft, role separation, validation, owner update/soft-withdraw and audit tests | PLANNED |
| UC-S2 | Submit Screening | FR-6.1/6.2; NFR-2/3/4 | `POST /api/v1/screenings/{id}/submit` | Screening Service, Rate Limiter, Idempotency Manager; SD-2 rate/auth/replay/completeness/locking/rollback tests | PLANNED |
| UC-S3 | Assign and Review Screening | FR-7; NFR-2/3/4 | Handler and review POST endpoints | Screening Service and role/review persistence; SD-3 phase/handler/score/single-review/transaction tests | PLANNED |
| UC-S4 | Decide and Finalize Screening | FR-8; NFR-2/3/4 | Decision, final-submission, schedule POST endpoints | Screening Service and Schedule Conflict Checker; SD-4 approve/reject/final-submit/auto-reject/conflict/locking tests | PLANNED |
| UC-S5 | Search and View Screenings | FR-10, FR-11; NFR-1/3 | Screening collection/detail GET | Search/Visibility Service and SQL projections; AD-5 filter/all-word/access/redaction/sort/page tests | PLANNED |
