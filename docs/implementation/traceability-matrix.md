...
Prompt 1 added the build, configuration, Flyway, JPA entity, repository, and database-independent domain-test foundation. Prompt 2 added and tested shared NFR-2 through NFR-4 infrastructure. All FR and use-case rows remain `PLANNED` because no Program/Screening business controller or orchestration service has been implemented. An NFR row marked `IMPLEMENTED` below claims its reusable shared foundation only; actual business-command integration and real-MySQL transaction/concurrency verification remain attached to the relevant planned FR/UC rows.
