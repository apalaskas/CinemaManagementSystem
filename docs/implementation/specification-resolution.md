...
- Idempotency keys use the restricted 1-255-character ASCII syntax recorded in `api-contract.md`. The database uniquely scopes each unexpired key by authenticated user; the stable uppercase operation (up to 100 characters) is stored and compared so cross-operation reuse is rejected. SHA-256 covers operation plus recursively key-sorted canonical JSON content. Records expire after a configurable retention period, defaulting to 24 hours.
- The academic rate limiter is a bounded, idle-expiring, in-process fixed-window implementation. It executes before HTTP Basic authentication to preserve the SD-2 order. It uses a trusted identity already present in the security context when available and otherwise trusts the servlet container's direct remote address, never arbitrary forwarding headers, for its key. A multi-instance deployment requires a shared limiter but does not justify Redis/container infrastructure in this project.
